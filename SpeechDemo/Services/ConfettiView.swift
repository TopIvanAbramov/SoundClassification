...
private let kAnimationLayerKey = "animation.confetti"
...
public final class ConfettiView: UIView {
    ...
    */
    public func emit(with contents: [Content],
                     ...
                     birthRate: Float = 20.0,
                     completion: ((Bool) -> Void)? = nil) {
        let layer = Layer()
        layer.configure(with: contents, andBirthRate: birthRate)
    ...
    private final class Layer: CAEmitterLayer {
        ...
        func configure(with contents: [Content], andBirthRate birthRate: Float) {
            emitterCells = contents.map { content in
                let cell = CAEmitterCell()

                cell.birthRate = birthRate / Float(contents.count)
                cell.lifetime = 10.0
                cell.velocity = CGFloat(birthRate * cell.lifetime)
...
extension ConfettiView.Content {
    var color: UIColor? {
        switch self {
